...
std::string signature(const ast::ExprPtr& expression) {
    switch (expression->kind) {
        case ast::Kind::Contact: { const auto& x = static_cast<const ast::ContactExpr&>(*expression); return "CONTACT(" + x.a + "," + x.b + ")"; }
        case ast::Kind::Distance: { const auto& x = static_cast<const ast::DistanceExpr&>(*expression); return "DISTANCE(" + x.a + "," + x.b + ")"; }
        case ast::Kind::ContactCount: { const auto& x = static_cast<const ast::ContactCountExpr&>(*expression); return "CONTACT_COUNT(" + x.a + "," + x.b + ")"; }
...
Predicate make_predicate(const ast::ExprPtr& expression, const Topology& topology) {
    if (expression->kind == ast::Kind::Contact) {
        auto observable = make_observable(expression, topology);
        return [observable](const Frame& frame) { return observable->evaluate(frame) != 0.0; };
    }
    if (expression->kind == ast::Kind::Comparison) {
        const auto& comparison = static_cast<const ast::ComparisonExpr&>(*expression);
        auto observable = make_observable(comparison.operand, topology);
        const double threshold = comparison.threshold; const auto op = comparison.op;
        return [observable, threshold, op](const Frame& frame) {
    ...
    if (expression->kind == ast::Kind::And || expression->kind == ast::Kind::Or) {
        ...
        auto left = make_predicate(binary.left, topology), right = make_predicate(binary.right, topology);
        const bool is_and = expression->kind == ast::Kind::And;
        return [left = std::move(left), right = std::move(right), is_and](const Frame& frame) {
            return is_and ? left(frame) && right(frame) : left(frame) || right(frame);
        };
    }
...
std::vector<Event> Engine::execute(Trajectory& trajectory, const ExecutionPlan& plan) const {
    ...
    for (const auto& [key, expression] : sources) {
        states.emplace(key, State{make_predicate(expression, trajectory.topology()), EventExtractor(key, expression_selections(expression)), {}});
    }
    ...
    while (trajectory.reader().next(frame)) {
        for (auto& [key, state] : states) {
            (void)key;
            if (auto event = state.extractor.push(frame.time_ps, state.predicate(frame))) state.events.push_back(std::move(*event));
        }
    }
